...
/**
 * Parse move description for extended critical hit range
 * Default is natural 20 only. Some moves crit on 19-20 or wider.
 *
 * @param {string} description - Move description text
 * @returns {number} Minimum natural roll for a critical hit (default 20)
 */
function parseCriticalRange(description) {
  if (!description) return 20;

  const descLower = description.toLowerCase();

  // Patterns to match:
  // "scores a critical hit on 19 and 20"
  // "critical hit on rolls of 19 or 20"
  // "scores a critical hit on 19s and 20s"
  // "critical hit on natural attack rolls of 19 or 20"
  // "critical on 19s and 20s"
  const critPatterns = [
    /critical\s+(?:hit\s+)?(?:on\s+)?(?:natural\s+)?(?:attack\s+)?(?:rolls?\s+)?(?:of\s+)?(\d+)\s*(?:and|or|s\s+and)\s*20/i,
    /scores?\s+(?:a\s+)?critical\s+(?:hit\s+)?on\s+(\d+)s?\s+and\s+20s?/i,
    /crit(?:ical)?\s+(?:hit\s+)?(?:on\s+)?(\d+)\s*(?:and|or)\s*20/i
  ];
  ...
  // Check for "18 or more" style patterns (less common)
  const thresholdPattern = /(?:natural\s+)?(?:attack\s+)?roll\s+(?:of\s+)?(\d+)\s+or\s+(?:more|higher)/i;
  const thresholdMatch = descLower.match(thresholdPattern);
  if (thresholdMatch) {
    // This is for status effects, not crits, so ignore
  }

  return 20; // Default: only natural 20 is a critical hit
}

/**
 * Check if a natural roll is a critical hit given a threshold
 *
 * @param {number} naturalRoll - The natural d20 roll (before modifiers)
 * @param {number} critThreshold - Minimum roll for critical (default 20)
 * @returns {boolean} True if critical hit
 */
function isCriticalRoll(naturalRoll, critThreshold = 20) {
