...
function getStruggleMove() {
  return {
    ...
    time: '1 action',
    range: 'Melee',
    description: 'A desperate attack used when all PP is exhausted. The user takes recoil damage.',
 ...
 *
 * @returns {number} Recoil damage amount
 */
...
function applyStruggleRecoil(combatant) {
  const recoilDamage = calculateStruggleRecoil();
  const hpBefore = combatant.current_hp;
  combatant.current_hp = Math.max(0, combatant.current_hp - recoilDamage);
