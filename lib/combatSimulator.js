...
import { evaluateFormulaWithRng } from './formulaEvaluator.js';
import {
...
import { AI_WEIGHTS } from './combatAI.js';
...
export function runNextTurn(simulation) {
  ...
  for (const { combatant, target, isPlayer, statsKey } of actors) {
    ...
    // AI selects a move (always uses tactical mode)
    const moveSelection = selectMove(combatant, target, simulation.rng);
    const move = moveSelection.move;

    // Log AI reasoning for all move selections
    if (moveSelection.reasoning.length > 0) {
      turnLog.push({
        type: 'ai_reasoning',
        actor: combatant.name,
        details: { reasoning: moveSelection.reasoning },
        formatted: `  [AI] ${moveSelection.reasoning.join(' | ')}`
      });
    }
    ...
    if (actionResult.is_save_move) {
    ...
    } else if (actionResult.is_ohko_move) {
      // Feature 029: OHKO move logging
      fullLog += '\n' + logger.logOHKOMove(
        combatant.name,
        move.name,
        target.name,
        actionResult.ohko_result,
        actionResult.target_hp_before,
        actionResult.target_hp_after
      );
    } else if (actionResult.is_formula_move) {
      // Feature 029: Formula move logging - show hit check first
      if (actionResult.attack_roll) {
        fullLog += '\n' + logger.logHitCheck(
      ...
      // Then show formula damage details
      if (actionResult.hit && actionResult.damage) {
        fullLog += '\n' + logger.logFormulaMove(
          combatant.name,
          move.name,
          target.name,
          actionResult.damage,
    ...
    } else if (actionResult.is_conditional_move) {
      // Feature 029: Conditional move logging - show hit check first
      if (actionResult.attack_roll) {
        fullLog += '\n' + logger.logHitCheck(
          actionResult.attack_roll.total,
          target.ac,
      ...
      // Then show conditional damage details
      if (actionResult.hit && actionResult.damage) {
        fullLog += '\n' + logger.logConditionalMove(
          combatant.name,
          move.name,
          target.name,
          actionResult.damage,
    ...
    } else if (actionResult.is_two_turn_charge) {
      // Feature 029: Two-turn move charging
      fullLog += '\n' + logger.logTwoTurnCharge(combatant.name, move.name, actionResult);
    } else if (actionResult.is_two_turn_resolve) {
      // Feature 029: Two-turn move resolution
      fullLog += '\n' + logger.logTwoTurnResolve(combatant.name, move.name);
      if (actionResult.attack_roll) {
        fullLog += '\n' + logger.logHitCheck(
          actionResult.attack_roll.total,
          target.ac,
    ...
    } else if (actionResult.is_invulnerable_miss) {
      // Feature 029: Attack missed due to invulnerability
      fullLog += '\n' + logger.logInvulnerableMiss(
        combatant.name,
        move.name,
        target.name,
        actionResult.invulnerable_reason
      );
    ...
    if (actionResult.hit && actionResult.damage && !actionResult.is_ohko_move && !actionResult.is_formula_move && !actionResult.is_conditional_move && !actionResult.is_two_turn_resolve) {
...
/**
 * Select a move for the combatant using tactical AI scoring
 *
 * @param {Object} combatant - Combatant selecting move
 * @param {Object} target - Target combatant
 * @param {Object} rng - Seeded RNG instance (unused, kept for API compatibility)
 * @returns {{ move: Object|null, reasoning: string[] }} Selected move and reasoning
 */
function selectMove(combatant, target, rng) {
  ...
  if (availableMoves.length === 0) {
    return { move: null, reasoning: ['No moves with PP available'] }; // Will use Struggle
  }

  // Always use tactical mode for consistent AI tuning
  return selectMoveTactical(combatant, target, availableMoves);
}

/**
 * Select move using tactical AI scoring
 * Considers post-movement range since AI can move AND attack in same turn
 *
 * @param {Object} combatant - Combatant selecting move
 * @param {Object} target - Target combatant
 * @param {Object[]} availableMoves - Moves with PP > 0
 * @returns {{ move: Object|null, reasoning: string[] }} Best move and reasoning
 */
function selectMoveTactical(combatant, target, availableMoves) {
  ...
  // Calculate current distance and movement budget
  const currentDistance = combatant.position && target.position
    ? getManhattanDistance(combatant.position, target.position)
    : 1; // Assume adjacent if no positions
  const movementBudget = combatant.has_moved_this_turn
    ? 0
    : (combatant.movement_remaining ?? combatant.walking_speed ?? 6);
  ...
  for (const move of availableMoves) {
    ...
    // Check PP
    const currentPp = combatant.move_pp?.[move.id] ?? move.pp ?? 0;
    if (currentPp <= 0) {
      continue; // Skip moves with no PP
    }
    if (currentPp <= 2) {
      score += AI_WEIGHTS.LOW_PP_WARNING;
      reasoning.push(`Low PP (${currentPp})`);
    }

    // Check if this is a self-target move (buff/utility)
    const isSelfTarget = isSelfTargetMove(move);

    // Check if this move deals damage (has damage dice in description)
    const isDamagingMove = checkIfDamagingMove(move);

    if (isSelfTarget) {
      // Self-target moves (Harden, etc.) - always "usable" but low priority
      // They don't deal damage, so heavily deprioritize vs attacking
      score += 20; // Low base score - usable but not preferred
      reasoning.push('Self-buff move');

      // Check if buff is already active (don't stack same buff)
      const buffName = move.name.toLowerCase();
      if (combatant.combat_buffs?.damage_reduction && buffName === 'harden') {
        score -= 100; // Already have this buff active
        reasoning.push('Buff already active');
      }
    } else if (!isDamagingMove) {
      // Non-damaging moves that target enemy (Growl, Leer, etc.)
      // These are useful but should not be preferred over attacks
      score += 50; // Lower than IN_RANGE (100) but usable
      reasoning.push('Status/debuff move');

      // Don't spam the same debuff - check if we've used it recently
      // (debuff moves should be used strategically, not every turn)
    } else {
      // Normal attack or status move - check range to target
      const moveRange = getMoveRange(move);

      // Check if in range NOW or AFTER movement
      ...
      if (inRangeNow) {
        ...
        reasoning.push('In range');
      } else if (inRangeAfterMove) {
        // Can reach after moving - slight penalty for needing to move but still viable
        score += AI_WEIGHTS.IN_RANGE - 20;
        reasoning.push(`In range after moving (${currentDistance - moveRange} cells needed)`);
      } else {
        // Cannot reach even after moving - heavy penalty
        score += AI_WEIGHTS.OUT_OF_RANGE;
        reasoning.push(`Out of range even after moving (need ${currentDistance - movementBudget} more cells)`);
      }

      // Type effectiveness (only for moves that target the enemy)
      const effectiveness = getEffectiveness(move.type, target.type);
      if (effectiveness.multiplier >= 4) {
        score += AI_WEIGHTS.TYPE_ADVANTAGE_4X;
        reasoning.push('Super effective (4x)');
      } else if (effectiveness.multiplier >= 2) {
        score += AI_WEIGHTS.TYPE_ADVANTAGE_2X;
        reasoning.push('Super effective (2x)');
      } else if (effectiveness.multiplier <= 0.25) {
        score += AI_WEIGHTS.TYPE_DISADVANTAGE_QUARTER;
        reasoning.push('Not very effective (0.25x)');
      } else if (effectiveness.multiplier <= 0.5) {
        score += AI_WEIGHTS.TYPE_DISADVANTAGE_HALF;
        reasoning.push('Not very effective (0.5x)');
      } else if (effectiveness.multiplier === 0) {
        score += -2000; // Immune - never use
        reasoning.push('Target is immune');
      }
      ...
      if (hpPercent <= 25) {
        ...
        reasoning.push('Target at low HP');
      }

      // Status move consideration (moves that inflict status on enemy)
      const isStatusMove = move.power === 'none' || move.power === null;
      if (isStatusMove) {
        const hasStatus = target.status_effects && target.status_effects.length > 0;
        if (hasStatus) {
          score += AI_WEIGHTS.STATUS_ON_STATUSED;
          reasoning.push('Target already has status');
        } else {
  ...
  // Add selection summary to reasoning
  const fullReasoning = [
    `Tactical: ${bestMove?.name || 'none'} (score: ${bestScore})`,
    ...bestReasoning
  ];

  return { move: bestMove, reasoning: fullReasoning };
}
...
function isSelfTargetMove(move) {
  ...
  // Pure "self" is a true self-target (Harden, Roost)
  if (range === 'self') return true;

  // "self (Xft cone/line/radius)" are AoE attacks that originate from self
  // but target enemies - these are NOT self-buffs
  if (range.startsWith('self')) {
    const isAoE = /cone|line|radius|sphere|cube|area|beam|blast/.test(range);
    if (isAoE) return false; // AoE moves target enemies, not self
  }
...
/**
 * Check if a move deals damage (has damage dice in description)
 * @param {Object} move - Move object
 * @returns {boolean} True if move deals damage
 */
function checkIfDamagingMove(move) {
  if (!move.description) return false;
  const desc = move.description.toLowerCase();

  // Check for damage dice patterns like "1d6", "2d8", etc.
  const hasDamageDice = /\d+d\d+/.test(desc);

  // Check for "damage on a hit" or "damage on a failed save"
  const hasDamageKeyword = desc.includes('damage');

  return hasDamageDice && hasDamageKeyword;
}
...
/**
 * Feature 029: Execute an OHKO move
 ...
 */
function executeOHKOMove(attacker, defender, move, rng, roundNumber) {
  ...
  // Roll d20 for OHKO using seeded RNG
  const naturalRoll = rng.rollD20();
...
/**
 * Feature 029: Execute a formula-based damage move
 ...
 */
function executeFormulaMove(attacker, defender, move, rng, roundNumber) {
  ...
  if (hit) {
    // Create a roll function using seeded RNG
    const rollFn = (sides) => rng.rollSingleDie(sides);

    // Evaluate the formula expression with seeded RNG
    const formulaResult = evaluateFormulaWithRng(formula.expression, context, rollFn);
    ...
    damage = {
      ...
      type_effectiveness: `${moveType} vs ${defender.type.join('/')}`,
...
/**
 * Feature 029: Execute a conditional damage move
 ...
 */
function executeConditionalMove(attacker, defender, move, rng, roundNumber) {
  ...
  if (hit) {
    // Get base damage from dice using seeded RNG
    const diceExpression = parseDamageDice(move.description, move.higherLevels, attacker.level);
    const diceResult = diceExpression ? rng.rollDiceDetailed(diceExpression) : { total: 0, rolls: [] };
    ...
    const conditionalResult = calculateConditionalMultiplier(attacker, defender, conditional);
...
/**
 * Feature 029: Execute two-turn move resolution (turn 2) with seeded RNG
 *
 * @param {Object} attacker - Attacking combatant
 * @param {Object} defender - Defending combatant
 * @param {Object} move - Move being resolved
 * @param {Object} rng - Seeded RNG
 * @param {number} roundNumber - Current round
 * @returns {Object} Attack result
 */
function executeTwoTurnResolve(attacker, defender, move, rng, roundNumber) {
  ...
  // Execute as normal attack with seeded RNG
  const attackRoll = calculateAttackRollWithRng(attacker, move, rng);
...
function executeAttack(attacker, defender, move, rng, roundNumber) {
  ...
  // Feature 029: Check if defender is invulnerable
  ...
  // Feature 029: Check if this is a two-turn move
  if (move.turns && move.turns.count === 2) {
    ...
    if (attacker.charging_move && attacker.charging_move.moveId === move.id) {
      // Turn 2: Resolve the attack using seeded RNG
      return executeTwoTurnResolve(attacker, defender, move, rng, roundNumber);
    } else {
  ...
  // Feature 029: Check if OHKO move
  if (move.ohko) {
    return executeOHKOMove(attacker, defender, move, rng, roundNumber);
  }

  // Feature 029: Check if formula-based damage move
  if (move.formula) {
    return executeFormulaMove(attacker, defender, move, rng, roundNumber);
  }

  // Feature 029: Check if conditional damage move
  if (move.conditional) {
    return executeConditionalMove(attacker, defender, move, rng, roundNumber);
  }
