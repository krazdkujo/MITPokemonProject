 ...
 * @param {string} config.aiMode - AI mode: 'random' or 'tactical' (default 'random')
 * @returns {Object} Simulation instance
 */
export function createSimulation(config) {
  const { pokemon1, pokemon2, seed = null, maxTurns = 100, aiMode = AI_MODE.RANDOM } = config;
  ...
  return {
    ...
    combatant1,
    combatant2,
    aiMode, // Store at top level for easy access
    state: 'ready',
    currentTurn: 0,
    log: [],
    seed: rng.seed,
...
export function runNextTurn(simulation) {
  ...
  for (const { combatant, target, isPlayer, statsKey } of actors) {
    ...
    // AI selects a move
    const moveSelection = selectMove(combatant, target, simulation.rng, simulation.aiMode);
    const move = moveSelection.move;

    // Log AI reasoning in tactical mode
    if (simulation.aiMode === AI_MODE.TACTICAL && moveSelection.reasoning.length > 0) {
      turnLog.push({
        type: 'ai_reasoning',
        actor: combatant.name,
        details: { reasoning: moveSelection.reasoning, aiMode: simulation.aiMode },
...
function selectMove(combatant, target, rng, aiMode) {
  ...
  if (aiMode === AI_MODE.RANDOM) {
    const index = Math.floor(rng.random() * availableMoves.length);
    return {
      move: availableMoves[index],
      reasoning: [`Random selection: ${availableMoves[index].name}`]
    };
  }

  // Tactical mode: Use weighted scoring from combatAI
