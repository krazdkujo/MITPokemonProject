 ...
 * Feature 029: Now uses structured fields from moves.json when available
 * @param {Object} move - Move object with description and optional structured fields (recoil, formula, etc.)
 * @returns {MoveEffect}
 */
export function parseMoveEffects(move) {
  ...
  // Feature 029: Prefer structured recoil field from moves.json over description parsing
  let recoil = null;
  if (move?.recoil) {
    // Use structured recoil data from extraction
    recoil = {
      percentage: move.recoil.percentage,
      typeless: move.recoil.type === 'typeless'
    };
  } else {
    // Fall back to description parsing for legacy support
